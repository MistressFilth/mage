...
- `MappingArtifactWatcher.run()` now wraps the `COSMETIC_WATCHER_STARTED` event emit, the catch-up `_handle_mapping_saved()` call, and the poll loop under a single `try:` / `finally:`. An uncaught exception during the catch-up (e.g., `pydantic.ValidationError` from a malformed `mapping.yaml`, or a refiner network error) previously left the audit trail dangling on `COSMETIC_WATCHER_STARTED` and the PID file on disk; the catch-up now exits the watcher cleanly with both `COSMETIC_WATCHER_STARTED` and `COSMETIC_WATCHER_STOPPED` in the log and the PID file removed. (P28b)
- `InspectLoopStage.inspect_increment` now emits `INSPECT_LOOP_PASSED`, `INSPECT_LOOP_FAILED`, or `INSPECT_LOOP_COMPLETED` at every exit point (budget exceeded, clean pass, spec route, code route, cosmetic-only), pairing each `INSPECT_LOOP_STARTED` with a completion marker. The previously dead `INSPECT_LOOP_*` enum members are now wired in. Downstream consumers of `events.jsonl` can now bracket one increment of inspect cleanly. (P28c)
- `SettleFeatureStage._execute_disposition` now emits `SETTLE_BRANCH_KEPT` on the `disposition="kept"` early-return path (symmetric to the existing `SETTLE_BRANCH_DISCARDED` for `discarded`). Previously the `kept` disposition returned silently without a branch-level audit-trail marker; downstream consumers of `events.jsonl` can now distinguish `kept` from `pr_opened`/`merged` cleanly. (P29)
- `EtchStage.run_scenario` now emits a final `ETCH_COMPLETED` after the per-step for-loop completes, with payload `{scenario_name, red_test_count: len(target.steps)}` and `reason="no_steps"` when the loop body did not run. Previously `run_scenario` returned silently for the empty-steps case (the per-step emit only runs inside the loop), leaving the audit trail dangling on `ETCH_STARTED`. Downstream consumers can now bracket one `run_scenario` invocation cleanly. (P29)
- Four docstring drifts corrected in `SettleFeatureStage`, `apply_for_feature`, `MappingArtifactWatcher.stop`, and `InspectLoopStage` / `inspect_loop.py` module docstrings — each now describes the actual code behavior. (P29)
