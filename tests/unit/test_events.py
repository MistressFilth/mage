...
class TestPlan5EventTypes:
    def test_inspect_feature_events_full(self):
        ...
        assert (
            EventType.INSPECT_FEATURE_HALT_PERSISTED.value
            == "inspect_feature_halt_persisted"
        )
...
class TestEventsLog:
    ...
    @pytest.mark.asyncio
    async def test_append_and_read_all(self, tmp_path: Path):
        log_path = tmp_path / "events.jsonl"
    ...
    @pytest.mark.asyncio
    async def test_log_is_append_only(self, tmp_path: Path):
